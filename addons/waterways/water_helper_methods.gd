...
static func generate_collisionmap(image: Image, mesh_instance: MeshInstance3D, raycast_dist: float, raycast_layers: int, steps: int, step_length_divs: int, step_width_divs: int, river, use_uv2: bool = true) -> Image:
	var space_state := mesh_instance.get_world_3d().direct_space_state
	# Support both UV and UV2
	var uv_index := Mesh.ARRAY_TEX_UV2 if use_uv2 else Mesh.ARRAY_TEX_UV
	var uv2 := mesh_instance.mesh.surface_get_arrays(0)[uv_index] as PackedVector2Array
